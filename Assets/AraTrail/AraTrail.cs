...
namespace Ara{
    ...
    public class AraTrail : MonoBehaviour {
    	...
    	private void UpdateVelocity(){
            ...
            if (Time.deltaTime > 0){
                velocity = Vector3.Lerp((transform.position - prevPosition) / Time.deltaTime,velocity,velocitySmoothing);
        ...
        private void Warmup(){
            ...
            while (simulatedTime > Time.fixedDeltaTime){

                PhysicsStep(Time.fixedDeltaTime);

                EmissionStep(Time.fixedDeltaTime);
        ...
        private void UpdateSegmentMesh(List<Point> input, int start, int end, Vector3 localCamPosition){
            ...
            if (trail.Count > 1){
                ...
                for (int i = trail.Count-1; i >= 0; --i){
                    ...
                    float normalizedLife =   1 - trail[i].life / time;
